...
A Rust reimplementation of Codex, built as a Cargo workspace with 5 crates. The project follows the design of the original TypeScript Codex but is implemented from scratch in Rust with async-first patterns.
...
# Build and run the CLI binary (named `rust-Codex`)
# With a prompt: runs the non-interactive query loop
cargo run -p rust-Codex-cli -- "your prompt here"

# Without a prompt: enters the basic TUI
cargo run -p rust-Codex-cli

# Run all tests across all crates
cargo test --workspace

# Run tests for a specific crate
cargo test -p rust-Codex-core
cargo test -p rust-Codex-api
cargo test -p rust-Codex-tools
cargo test -p rust-Codex-cli
cargo test -p rust-Codex-tui

# Run a single test by name
cargo test -p rust-Codex-core -- test_name

# Check without building
cargo check --workspace
cargo check -p rust-Codex-tui
cargo check -p rust-Codex-cli

# Integration tests (ignored by default, require ANTHROPIC_API_KEY or local endpoint)
cargo test -p rust-Codex-api -- --ignored
cargo test -p rust-Codex-cli -- --ignored
```

## Environment Variables

- `ANTHROPIC_API_KEY` — API key for x-api-key authentication
- `ANTHROPIC_AUTH_TOKEN` — Auth token for Bearer authentication (used by Codex)
- `ANTHROPIC_MODEL` — Override the model (also read from `~/.Codex/settings.json` env)
- `ANTHROPIC_BASE_URL` — Custom API endpoint URL
- `CLAUDE_CONFIG_DIR` — Override Codex config directory (default: `~/.Codex`)
...
This means rust-Codex works out-of-the-box in a configured Codex environment — it reads `~/.Codex/settings.json` env variables (including `ANTHROPIC_AUTH_TOKEN` and `ANTHROPIC_BASE_URL`) and auto-selects the correct auth mode.

Config file location: `~/.config/rust-Codex/config.json`
Codex settings: `~/.Codex/settings.json` (reads `env`, `model`, `apiKeyHelper` fields)
Permission rules location: `~/.config/rust-Codex/permissions.json`

## CLI Parameters

```
rust-Codex [OPTIONS] [PROMPT]...
...
Options:
  ...
  -p, --print                        Print response and exit (non-interactive mode)
      ...
      --settings <PATH>              Path to Codex settings.json
```
...
- **`tools`** — Tool trait and implementations: `BashTool`, `FileReadTool`, `FileEditTool`, `FileWriteTool`, `TodoWriteTool`, `ToolSearchTool`, and lower-frequency extended tools. Each implements the `Tool` trait (`info()`, `is_read_only()`, `is_concurrency_safe()`, `should_defer()`, `execute()`). `ToolRegistry` indexes tools by name, partitions them by concurrency safety and deferred-loading status, supports deferred-tool keyword search, estimates deferred schema size, and supports allow/deny list filtering via `apply_tool_filters()`.
- **`cli`** — Binary crate (`rust-Codex`) and `QueryLoop` — the agentic loop that sends messages, streams responses, detects `tool_use` stop reasons, checks permissions, executes tools (concurrent-safe in parallel via `join_all`, others serially), appends `tool_result` messages, and loops up to `max_rounds` (default 8). CLI supports extensive parameters (`--model`, `--mode`, `--print`, `--max-turns`, `--system-prompt`, etc.) and a unified priority chain for configuration resolution. With a prompt argument it runs non-interactively; without a prompt it enters the basic TUI and dispatches prompts through the same `QueryLoop` in a background task.
...
- **Deferred tool loading** — When deferred tools' estimated schema size exceeds 10% of the model context window, initial API requests send full schemas for non-deferred tools and minimal `{ name, deferred: true }` entries for undiscovered deferred tools. The always-available `ToolSearch` tool lets the model discover full schemas by keyword or `select:ToolName`; actually invoking a deferred tool promotes it to full-schema requests for the rest of the session.
