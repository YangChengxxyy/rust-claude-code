...
- 在 `core` 内固定 `always_deny > always_allow > permission_mode` 的判断顺序，并保留 `Plan` 模式的硬只读限制。
...
- 当前 `AppState` 已包含：
  ...
  - `todos`
  - `permission_mode`
  - `session`（当前收敛 `model` 与 `max_tokens`）
  - `cwd`
  - `total_usage`
...
- 当前已先收敛出最小 `session` 配置对象，避免 `AppState` 继续平铺更多会话字段。
...
- 会话配置当前仅收敛 `model` 与 `max_tokens`，不提前加入 system prompt、provider、重试策略等未落地状态。
...
- 当前客户端实现能工作，且已把 endpoint、header 与 JSON 请求发送路径收敛为共享内部入口；但流式与非流式的执行边界仍未完全拆开。
- 当前类型层已经支持 `stream`，客户端也已为后续流式复用预留共享请求入口，但 SSE 解析与流式响应处理仍未实现。
