...
> 最新核对: 2026-05-19
>
> 第四期已全部完成。迭代 35-40、42-43 已收口；迭代 41 已移出当前系统范围，不计入第四期交付。
...
**2026-05-19 最新状态**：第四期已全部完成。迭代 35-40、42-43 已完成；迭代 41（OAuth / `@include`）已明确不纳入当前系统范围，不计入第四期交付。后续开发以 `doc/phase5-iteration-plan.md` 为准。
...
- 已补齐沙箱配置、平台适配和命令包装路径。
- 已补齐 `PermissionMode::Auto` 与 `--mode auto` 基础自动审批语义。
- SDK 已提供可嵌入使用的公共 `Session` API、builder 和响应流接口。
...
- TUI Task 视图面板已补齐。
- WebSearch 已支持 Tavily / SearXNG 多后端配置。
- TypeScript/TSX 语法高亮已细化。
- `PermissionMode::Auto` 已落地。
- `/plugin install` / `/plugin remove` 已实现本地插件安装删除流程。
...
- [x] `--sandbox` 模式下 Bash 命令受限
- [x] SDK `Session` API 可独立使用
- [x] 所有 Phase 2-3 遗留项清零
...
Stage A、Stage B 与 Stage C 均已完成。第四期已收口，后续进入第五期兼容性补齐阶段：

1. 按 `doc/phase5-iteration-plan.md` 从迭代 44 开始推进。
2. 第五期优先处理文件工具 schema 兼容、工具 schema 契约测试和 `stream-json` 输出协议。
